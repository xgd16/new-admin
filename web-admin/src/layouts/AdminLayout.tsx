...
import { Popover, Text } from '@heroui/react'
...
import { useSidebarCompact, useSidebarFolded } from '../prefs/workspace'
import { useTheme } from '../theme/themeContext'

function pathMatchesLeaf(pathname: string, leafPath: string) {
  return pathname === leafPath || pathname.startsWith(`${leafPath}/`)
}

function groupHasActiveDescendant(g: AdminNavGroup, pathname: string): boolean {
...
export function AdminLayout() {
  ...
  const sidebarIconRail = sidebarFolded
  const lgGridCols = sidebarIconRail
  ...
  return (
    <main className="app-shell min-h-svh w-full px-2 py-3 text-[color:var(--text)] sm:px-3 sm:py-5 lg:px-4">
      <div className="orb orb-primary -left-32 top-10" />
      <div className="orb orb-cyan -right-24 top-24" />
      <div className="soft-grid absolute inset-x-0 top-0 h-80 opacity-45" />

      <div
        className={`relative grid w-full min-w-0 max-w-none gap-3 sm:gap-5 lg:items-start ${lgGridCols}`}
      >
        <motion.aside
          className={`glass-panel hidden lg:sticky lg:top-5 lg:z-10 lg:flex lg:min-h-[calc(100svh-2.5rem)] lg:max-h-[calc(100svh-2.5rem)] lg:flex-col lg:self-start lg:overflow-hidden ${asidePadRadius}`}
        ...
        <section
          className={`flex min-w-0 max-w-full flex-col self-start ${sidebarCompact ? 'gap-2 sm:gap-4' : 'gap-3 sm:gap-5'}`}
        >
          <AdminHeader
            compact={sidebarCompact}
            logout={logout}
    ...
    >
      ...
      <div className={toolbarWrap}>
        ...
        <div className={userCluster}>
          ...
          <MotionButton className={logoutBtn} onClick={logout} title="退出登录">
